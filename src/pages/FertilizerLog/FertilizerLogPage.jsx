...
export default function FertilizerLogPage() {
  const { user, fields, fertilizerProducts, fertilizerLogs, setFertilizerProducts, setFertilizerLogs } = useStore()
  ...
  const perHectare = selectedField && logForm.quantityKg && selectedField.areaHa > 0
      ...
      <div className={styles.logCol}>
            ...
            <div className={styles.formGrid}>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>
                  地块<span className={styles.required}>*</span>
                </label>
                <select
              ...
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>
                  肥料产品<span className={styles.required}>*</span>
                </label>
                <select
                  className={styles.formSelect}
              ...
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>
                  用量 (公斤)<span className={styles.required}>*</span>
                </label>
                <input
        ...
        <div className={styles.historySection}>
          ...
          <div className={styles.filterRow}>
            <select className={styles.filterSelect} value={filterField} onChange={e => setFilterField(e.target.value)}>
            ...
            <button className={styles.btnReset} onClick={() => { setFilterField(''); setFilterProduct(''); setFilterDateFrom(''); setFilterDateTo('') }}>
              重置
            </button>
          </div>
        ...
        <Modal
          ...
          footer={
            <div className={styles.modalFooter}>
              <button className={styles.btnCancel} onClick={() => { setShowProductModal(false); setProductForm(EMPTY_PRODUCT) }}>
                取消
              </button>
              <button
        ...
        >
          <div className={styles.modalFormGrid}>
            <div className={`${styles.modalFormGroup} ${styles.modalFormGroupFull}`}>
              ...
              <input className={styles.modalInput} value={productForm.name} onChange={e => setProductForm(p => ({ ...p, name: e.target.value }))} placeholder="例：绿色高钾复合肥" />
            </div>
            <div className={styles.modalFormGroup}>
              <label className={styles.modalLabel}>品牌</label>
              <input className={styles.modalInput} value={productForm.brand} onChange={e => setProductForm(p => ({ ...p, brand: e.target.value }))} placeholder="例：YaraLiva" />
            </div>
            <div className={styles.modalFormGroup}>
              <label className={styles.modalLabel}>肥料类型</label>
              <select className={styles.modalSelect} value={productForm.type} onChange={e => setProductForm(p => ({ ...p, type: e.target.value }))}>
                {FERTILIZER_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div className={styles.modalFormGroup}>
              <label className={styles.modalLabel}>氮 (N) %</label>
              <input type="number" min="0" max="100" className={styles.modalInput} value={productForm.n} onChange={e => setProductForm(p => ({ ...p, n: e.target.value }))} placeholder="0" />
            </div>
            <div className={styles.modalFormGroup}>
              <label className={styles.modalLabel}>磷 (P) %</label>
              <input type="number" min="0" max="100" className={styles.modalInput} value={productForm.p} onChange={e => setProductForm(p => ({ ...p, p: e.target.value }))} placeholder="0" />
            </div>
            <div className={styles.modalFormGroup}>
              <label className={styles.modalLabel}>钾 (K) %</label>
              <input type="number" min="0" max="100" className={styles.modalInput} value={productForm.k} onChange={e => setProductForm(p => ({ ...p, k: e.target.value }))} placeholder="0" />
            </div>
            <div className={styles.modalFormGroup}>
              <label className={styles.modalLabel}>每公斤价格 (RM)</label>
              <input type="number" min="0" step="0.01" className={styles.modalInput} value={productForm.pricePerKg} onChange={e => setProductForm(p => ({ ...p, pricePerKg: e.target.value }))} placeholder="0.00" />
            </div>
            <div className={styles.modalFormGroup}>
              <label className={styles.modalLabel}>供应商</label>
              <input className={styles.modalInput} value={productForm.supplier} onChange={e => setProductForm(p => ({ ...p, supplier: e.target.value }))} placeholder="供应商名称" />
            </div>
            <div className={`${styles.modalFormGroup} ${styles.modalFormGroupFull}`}>
              <label className={styles.modalLabel}>备注</label>
              <textarea className={styles.modalTextarea} value={productForm.notes} onChange={e => setProductForm(p => ({ ...p, notes: e.target.value }))} placeholder="其他备注..." />
            </div>
          </div>
        </Modal>
      )}
